...
export class QDollarRecognizer {
  ...
  constructor() {
    ...
    this.PointClouds[7] = new PointCloud(
      'exclamation',
      new Array(new Point(526, 142, 1), new Point(526, 204, 1), new Point(526, 221, 2))
    )
...
//
function CloudMatch(candidate: PointCloud, template: PointCloud, minSoFar: number) {
...
function ComputeLowerBound(pts1: Point[], pts2: Point[], step: number, LUT: number[][]) {
  ...
  for (var i = step, j = 1; i < n; i += step, j++) LB[j] = LB[0] + i * SAT[n - 1] - n * SAT[i - 1]
  return LB
}
...
function MakeIntCoords(points: Point[]) {
  for (var i = 0; i < points.length; i++) {
...
function SqrEuclideanDistance(pt1: Point, pt2: Point) {
  var dx = pt2.X - pt1.X
  var dy = pt2.Y - pt1.Y
  return dx * dx + dy * dy
}
